...
| Cấu trúc thư mục | Nhóm theo số (`DL-20260918-004`): 1x INPUT · 2x PROCESS · 3x OUTPUT · 9x quản trị. Gói: quan-tri 1.8, ke-hoach 3.9, theo-doi-cv 1.7, soan-thao-vb 1.10 (24/9), the-thuc 1.0 · plugin **1.0.1** (24/9: `doi_soat_so_lieu.py` nhận đơn vị/loại/kỳ từ phần đầu tệp — bố cục `10-Dau-Vao` 21/9 không còn tên theo mã đơn vị) |
| KTC-Database & Google Drive | **Bản gốc trên Google Drive** `My Drive/KTC-Database` (máy này: ổ `H:`). Bản chép `D:` sẽ bị xóa, **đã cũ** (thiếu 163/1.171 tệp, 18/9/2026) — script đọc bản Drive qua `29-Cong-Cu/duong_dan.py`. KTC-Quan-tri vẫn chạy tại máy. Không ghi cứng ổ đĩa (C13), kiểm trùng C12 — Nguyên tắc 4, `DL-20260918-005` |
| Đơn vị nộp hồ sơ | Qua khung chat/project của chính họ (plugin Team) → tải sản phẩm về → gửi P-THHC. Tên tệp `<mã đơn vị>_<loại>_<kỳ>_v<N>` + phiếu tự kiểm. Không có hộp nhận tự động — `DL-20260919-001` |
...
| Tự học | Hook `UserPromptSubmit` từ plugin **1.3.0** mặc định chỉ ghi độ dài + nhãn tín hiệu `sua-sai`/`quy-uoc`/`quyet-dinh`; **nội dung chỉ khi chọn**: mở đầu `#học` hoặc biến `KTC_NHAT_KY_NOI_DUNG=1` (máy này đã bật ở `.claude/settings.local.json`; chỉ ghi lời có tín hiệu, che số định danh/SĐT/email); `#riêng` không ghi; tệp > 30 ngày tự xóa. Agent `ktc-tu-hoc` rút tri thức vào `05-Tri-Thuc-Tu-Hoc/TRI-THUC.md` — `DL-20260919-005`, `DL-20260926-001` |
| Plugin 1.3.0 | **26/9/2026** (`DL-20260926-001`, `KI-019`): không còn backup GitHub trong plugin (Task Scheduler chạy từ `29-Cong-Cu/plugin_src/scripts/`); guard `ktc_guard.py` PreToolUse chặn ghi KTC-Database/03-Templates/04-Good-Documents; chuẩn chung `20-Chuan-Chung/20-Quy-Tac-Bat-Bien-Va-Khuon-Dau-Ra.md` chèn vào 8 skill + 7 agent khi dựng (6 trạng thái DAT…KHONG_DAT); quan-tri 1.13. Bằng chứng tự sinh bằng `29-Cong-Cu/lap_bang_chung_plugin.py` (chạy sau mỗi lần dựng); nghiệm thu Code bằng `29-Cong-Cu/chay_nghiem_thu_code.py` (`claude plugin eval`, cần bật `defaultEnabled` trên bản sao, không cấp shell vì Windows không có sandbox). Máy phát triển cài plugin từ `~/.claude/ktc-marketplace/` — nâng bản: đổi tên thư mục cũ thành `.bak-<pb>` rồi chép `31-Plugin` (không xóa) |
...
| Đề án Trung tâm Đăng kiểm XCG | Hồ sơ ký 25/8/2026 (TTr 22 · Đề án 08 · BC 382, 384), **đã trình UBND tỉnh, chờ thẩm định**. Vốn điều lệ = kinh phí **22.688,60 triệu**, không góp vốn bằng đất, hoàn vốn **5,2 năm**; số tài chính do Phó Trưởng phòng TH-HC&QT chốt, AI không ghi đè. Toàn bộ tri thức: `90-Nhat-Ky-Van-Hanh/07-Tri-Thuc-Chuyen-De/De-an-Trung-tam-Dang-kiem-XCG.md`; điểm lệch còn mở: `KI-016` |
| CTMTQG NTM–GNBV–DTTS 2026–2030 | Vốn ĐTC 2026 giao Trường theo NQ 56/NQ-HĐND (28/8/2026) = **260 triệu** (NSTW 200 + NS tỉnh 60, "chuẩn bị đầu tư"), so dự toán CBĐT QĐ 1899 = 6.249,856 triệu; không có vốn sự nghiệp. Ba đại lượng không trộn: TMĐT (238.357 → đề xuất lần 2: 24.894) · dự toán CBĐT · vốn giao. Tri thức: `90-Nhat-Ky-Van-Hanh/07-Tri-Thuc-Chuyen-De/CTMTQG-NTM-GNBV-DTTS-2026-2030.md`; điểm lệch: `KI-018` |
| CTMTQG hiện đại hóa GD&ĐT 2026–2030 | Trường **đề xuất tham gia Dự án TP 2** (CV 405, 3/6/2026; UBND tỉnh chọn — CV 6095, 25/6) và DA TP 4. Đề xuất hiện hành (CV 642/643, 8/9/2026): **DA2 192,857 tỷ (NSTW 150)** + **DA4 15,125 tỷ thường xuyên**; QĐ 1951 chuẩn bị đầu tư 5,057 tỷ. **Không lẫn** với CTMTQG NTM-GNBV (BC 326, QĐ 1899, 20 tỷ) và Đề án 07 (409,374 tỷ). Số liệu đầu vào chốt 30/6/2026 — quy mô đào tạo có 3 số khác nhau (2.666/3.296/3.147). Toàn bộ: `90-Nhat-Ky-Van-Hanh/07-Tri-Thuc-Chuyen-De/CTMTQG-Hien-dai-hoa-GDDT-2026-2030.md`; điểm lệch: `KI-017` |
| Backup GitHub — kho 897 | Tác vụ thứ hai `KTC-897-Backup-GitHub` **21:05** hằng ngày, cùng script, `--du-an` trỏ kho `KTC-Ra-Soat-897-Universal-Plugin` (20/9/2026). Script nay dùng chung nhiều kho: **nơi ghi nhật ký tùy dự án** (có `90-Nhat-Ky-Van-Hanh/` thì ghi vào đó, không thì `.git/ktc-backup-log.jsonl`) — trước đây `makedirs` vô điều kiện nên sẽ để lại thư mục mang hình dạng KTC-Quan-tri trong kho khác rồi commit luôn lên GitHub. **Chốt bảo vệ theo chế độ gọi**: không chỉ định `--du-an` (hook SessionStart, chạy trong bất kỳ dự án nào người dùng mở) thì chỉ backup dự án có `90-Nhat-Ky-Van-Hanh/`; chỉ định rõ `--du-an` (Task Scheduler) thì chỉ cần là kho git |
| Backup GitHub tự động | Task Scheduler `KTC-Quan-tri-Backup-GitHub` 21:00 chạy bằng **`pythonw.exe`** — pythonw **không có std handle hợp lệ**, tiến trình con thừa kế stdin hỏng nên Git Credential Manager không trao đổi được thông tin đăng nhập: `git push` thất bại và **không in ra lỗi nào**. Hỏng lặng lẽ 18–20/9/2026, chạy tay thì luôn được. Vá bằng `stdin=subprocess.DEVNULL` trong `git()` (20/9/2026). Bộ thử cũ dùng remote cục bộ nên không chạm credential helper — đã thêm ca chạy dưới pythonw |
| Bản phát hành 1.0.0 | **21/9/2026**: cả hai hệ lên `1.0.0`, hết giai đoạn 0.x/alpha. `dong_goi_plugin.py` nay tự đóng `.zip` **lặp lại được** (mốc thời gian cố định trong ZipInfo, duyệt tệp theo thứ tự đã sắp) kèm tệp `.sha256`; loại `desktop.ini`, `__pycache__`, `.pyc` và **mọi archive lồng** (`.zip`/`.skill`). Gói ra `30-Ket-Qua/<ngày>/Plugin/`. Hướng dẫn cài Cowork + checksum: `KTC-Ra-Soat-897-Universal-Plugin/deployment-team/00-HUONG-DAN-CAI-COWORK.md` |
| Gói cũ | Mỗi hệ chỉ giữ **gói hiện hành + 1 bản trước** (để hoàn tác nhanh). Bản cũ hơn → `99-Luu-Tru/Goi-skill-cu/`; plugin zip cũ → `99-Luu-Tru/Goi-plugin-cu/` (dọn 19/9/2026, còn đủ trong lịch sử git) |
| Plugin `ktc-quan-tri` | **0.9.1** (19/9/2026, đã cài vào cache) — 5 skill + agent `ktc-tu-cai-tien` (chỉ đề xuất) + hook tự ghi nhật ký `04-Nhat-Ky-Tu-Dong/` và nạp vào context + backup GitHub 21:00 (Task Scheduler `KTC-Quan-tri-Backup-GitHub`). Remote: `github.com/nnqp201-sys/KTC-Quan-tri` (private, tài khoản `nnqp201-sys`), push lần đầu 18/9/2026 — `DL-20260918-002`. Thư mục dữ liệu nền nay là `11-Du-lieu-Cong-Viec` (bỏ tiền tố KTC). `Claude outputs/` không backup. Build: `29-Cong-Cu/dong_goi_plugin.py` |
