...
| Gói `ktc-bao-cao` | **`ktc-bao-cao-v3.10.skill`** (18/9/2026) — Task_ID (`DL-20260918-003`), kết cấu thư mục (`-004`), Nguyên tắc 4 (`-005`). Gói cũ giữ để hoàn tác, **không dùng** |
...
| Cấu trúc thư mục | Nhóm theo số (`DL-20260918-004`): 1x INPUT · 2x PROCESS · 3x OUTPUT · 9x quản trị. Gói: quan-tri 1.4, ke-hoach 3.5, theo-doi-cv 1.3, soan-thao-vb 1.4 |
| KTC-Database & Google Drive | Ngang cấp trong cùng thư mục Drive dùng chung; bản ổ `D:` sẽ bị xóa → **không ghi cứng ổ đĩa** (C13). Mỗi loại đầu vào một nơi lưu, kiểm trùng C12 — Nguyên tắc 4, `DL-20260918-005` |
| Plugin `ktc-quan-tri` | **0.5.0** (18/9/2026, đã cài vào cache) — 5 skill + agent `ktc-tu-cai-tien` (chỉ đề xuất) + hook tự ghi nhật ký `04-Nhat-Ky-Tu-Dong/` và nạp vào context + backup GitHub 21:00 (Task Scheduler `KTC-Quan-tri-Backup-GitHub`). Remote: `github.com/nnqp201-sys/KTC-Quan-tri` (private, tài khoản `nnqp201-sys`), push lần đầu 18/9/2026 — `DL-20260918-002`. Thư mục dữ liệu nền nay là `11-Du-lieu-Cong-Viec` (bỏ tiền tố KTC). `Claude outputs/` không backup. Build: `29-Cong-Cu/dong_goi_plugin.py` |
