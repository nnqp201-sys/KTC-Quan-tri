...
- Kho `KTC-Database` **chỉ đọc** — hook chặn ghi. Đề xuất sửa kho viết ra `12-Output/`.
...
- **Nguồn rời trong `references/` và nội dung trong `.skill` KHÔNG mặc nhiên đồng bộ — gói có thể MỚI HƠN.**
  ...
  trong gói là v3.0 còn bản rời chỉ v2.3. Dùng `tools/dong_goi_skill.py` (kiểm frontmatter, liên kết gãy,
...
- **Không hệ nào được sao chép bộ quy tắc của hệ khác để "cho đầy đủ".** Bản sao không có cơ chế đồng bộ
  ...
  với 10.513 b). Ngoại lệ duy nhất: 5 tệp dùng chung, bản gốc ở `01-Chuan-Chung/`, nhân bản tại bước đóng gói.
...
- **Chạy thử hệ thống**: `python tools/kiem_tra_he_thong.py` (Tầng 1, 11 phép kiểm) và bộ thử ngược
  `99-Kinh-Nghiem/02-Regression/Cases/test_kiem_tra_he_thong.py`. Ba tầng kiểm thử mô tả ở
  `99-Kinh-Nghiem/02-Regression/README.md`. Chạy sau mỗi lần sửa skill và **cả trước lẫn sau** khi đóng gói.
...
- **Bốn nguyên tắc soạn thảo bất biến** (`01-Chuan-Chung/14-Nguyen-Tac-Soan-Thao-Bat-Bien.md`) — áp dụng
...
- **Track Changes đã có công cụ thi hành**: `tools/ktc_trackchanges.py`, kích hoạt theo
  `01-Chuan-Chung/15-Skill-Track-Changes.md` khi người dùng nói "track changes" hoặc "ghi nhật ký sửa
...
- **Hai tệp "mẫu" trong `KTC-Bao-Cao/` không phải mẫu trống**: `00. Phu luc chi tiet...xlsx`/`.xltx` có
...
| Kho đầu vào | **`11-Du-Lieu-Dau-Vao/`** (14/9/2026) — dùng chung mọi hệ, 4 nhánh theo nguồn gốc. `Nhap_Ke_Hoach/` đã bỏ; `Nhap_Bao_Cao/` **chưa chuyển** — `DL-20260914-003` |
...
| Gói `ktc-bao-cao` | **`ktc-bao-cao-v3.8.skill`** (18/9/2026) — `read_bc736_excel.py` v3.3 đọc cột `Task_ID` (KI-001, `DL-20260918-003`) + sửa lỗi bỏ mất nhiệm vụ "Tổng hợp…". Các gói cũ (`v3.7` trở về trước) giữ lại để hoàn tác, **không dùng** |
...
| Plugin `ktc-quan-tri` | **0.3.0** (18/9/2026, đã cài vào cache) — 5 skill + agent `ktc-tu-cai-tien` (chỉ đề xuất) + hook tự ghi nhật ký `04-Nhat-Ky-Tu-Dong/` và nạp vào context + backup GitHub 21:00 (Task Scheduler `KTC-Quan-tri-Backup-GitHub`). Remote: `github.com/nnqp201-sys/KTC-Quan-tri` (private, tài khoản `nnqp201-sys`), push lần đầu 18/9/2026 — `DL-20260918-002`. Thư mục dữ liệu nền nay là `Du-lieu-Cong-Viec` (bỏ tiền tố KTC). `Claude outputs/` không backup. Build: `tools/dong_goi_plugin.py` |
...
2. `SKILL.md` và các tệp trong `01-Chuan-Chung/`.
...
Xem `99-Kinh-Nghiem/05-Known-Issues/Pending.md`. Không sao chép danh sách sang đây để tránh hai nơi lệch
trạng thái.

## Lịch sử

- Quyết định kiến trúc: `99-Kinh-Nghiem/06-Decision-Log/`
- Release notes: `99-Kinh-Nghiem/04-Release-Notes/`
- Bài học: `99-Kinh-Nghiem/01-Lessons-Learned/`
- Kết quả chạy thử: `12-Output/<ngày>/`
