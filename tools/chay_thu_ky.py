...
Chay: python tools/chay_thu_ky.py            # cham diem, in bang
      python tools/chay_thu_ky.py --ghi      # cham diem + ghi 1 dong vao BANG-CHI-SO.md
...
MAU = os.path.join(DU_AN, "KTC-Bao-Cao", "00. Mau bao cao thang (cap Truong).docx")
BANG = os.path.join(DU_AN, "99-Kinh-Nghiem", "02-Regression", "BANG-CHI-SO.md")
...
def tim_san_pham(ngay: str):
    d = os.path.join(DU_AN, "12-Output", ngay)
...
def main():
    ...
    if not ngay:
        ds = sorted(os.listdir(os.path.join(DU_AN, "12-Output")))
        ngay = ds[-1] if ds else None
    sp = tim_san_pham(ngay) if ngay else None
    if not sp or not all(sp):
        print(f"✗ Không tìm đủ 3 sản phẩm trong 12-Output/{ngay}/ — chạy pipeline trước:")
        print("    python tools/trich_tuong_thuat.py && python tools/build_bc2.py "
              "&& python tools/build_xl.py")
        return 1
    bc, pl, kh = sp
    print("=" * 78)
...
def ghi_bang(ngay, kq, n_dat):
    ...
    ver = "—"
    p = os.path.join(DU_AN, "KTC-Bao-Cao", "ktc-bao-cao-v3.5.skill")
    if os.path.exists(p):
        ver = "v3.5"
