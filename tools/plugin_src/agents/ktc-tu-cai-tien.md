...
1. **Thu dấu vết** (chỉ đọc):
   - `03-Nhat-Ky-Van-Hanh/04-Nhat-Ky-Tu-Dong/*.jsonl` — 7–14 ngày gần nhất: thao tác lỗi (`"loi": true`),
     tệp bị sửa đi sửa lại nhiều lần, backup thất bại.
   - `03-Nhat-Ky-Van-Hanh/03-Process-Memory/` — mục `key_findings`, `lessons` còn `approval_status: proposed`.
   - `99-Kinh-Nghiem/05-Known-Issues/Pending.md`, `06-Decision-Log/`, `01-Lessons-Learned/`,
     `03-Change-Proposals/` (để **không đề xuất trùng** cái đã có).
   - Chạy `python tools/kiem_tra_he_thong.py --chi-tiet` và ghi lại lỗi/cảnh báo.
...
3. **Viết đề xuất** vào `99-Kinh-Nghiem/03-Change-Proposals/CP-YYYYMMDD-NNN-<Ten-ngan>.md`
