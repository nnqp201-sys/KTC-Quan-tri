...
Chi hoat dong khi thu muc du an co `03-Nhat-Ky-Van-Hanh/` (tuc la dang lam viec trong KTC-Quan-tri) —
...
THU_MUC_LOG = os.path.join("03-Nhat-Ky-Van-Hanh", "04-Nhat-Ky-Tu-Dong")
...
def tim_du_an(data: dict):
    for p in (os.environ.get("CLAUDE_PROJECT_DIR"), data.get("cwd"), os.getcwd()):
        ...
        for _ in range(5):
            if os.path.isdir(os.path.join(p, "03-Nhat-Ky-Van-Hanh")):
                return p
...
def che_do_nap(data: dict):
    ...
    pm = moi_nhat(os.path.join(du_an, "03-Nhat-Ky-Van-Hanh", "03-Process-Memory"))
    cp = moi_nhat(os.path.join(du_an, "99-Kinh-Nghiem", "03-Change-Proposals"))
    dl = moi_nhat(os.path.join(du_an, "99-Kinh-Nghiem", "06-Decision-Log"))
