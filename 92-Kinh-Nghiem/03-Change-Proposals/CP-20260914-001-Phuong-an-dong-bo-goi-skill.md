...
**Phạm vi kiểm tra:** 5 gói `.skill` đang có × toàn bộ nguồn rời × `20-Chuan-Chung/`
...
`29-Cong-Cu/kiem_tra_he_thong.py` phép kiểm **C5** in ra:
...
| Tệp dùng chung | `20-Chuan-Chung` (gốc) | Trong `ktc-bao-cao-v3.5.skill` | Trong `ktc-ke-hoach-v3.1.skill` |
|---|---|---|---|
...
hàm chỉ so `20-Chuan-Chung/<tệp>` với `<hệ>/references/Skill-Library/<tệp>` — tức là **chỉ so bản rời**.
...
| | Trục 4 gọi là |
|---|---|
| Bản chuẩn `20-Chuan-Chung` (56 dòng) | "Xây dựng Đảng và hệ thống chính trị trong sạch, vững mạnh; **giữ gìn đoàn kết, thống nhất**" |
...
đối chiếu md5 với `20-Chuan-Chung/`:

| Trạng thái | Số tệp | Danh sách |
|---|---|---|
| Trùng khớp bản gốc ở `20-Chuan-Chung` | 4 | `11-Skill-Phan-Loai-6-Truc` · `12-Bang-Ma-Don-Vi` · `20-Tu-Dien-Truong-Du-Lieu` · `21-Quy-Tac-Task-ID` · `22-Vong-Doi-Va-Canh-Bao` |
...
Điều này mâu thuẫn với quy tắc của chính dự án — *"`20-Chuan-Chung/` giữ bản gốc, các hệ giữ bản sao"*.
...
Đọc `29-Cong-Cu/dong_goi_skill.py` thì rõ: quy trình đóng gói **không phải** "gom thư mục nguồn rời rồi nén".
Nó là:
...
**1.1. Mở rộng C5 để soi vào trong gói.** Đổi `c5_tep_dung_chung()` thành so bản gốc `20-Chuan-Chung` với
...
**1.4. Ca thử ngược bắt buộc** — `92-Kinh-Nghiem/02-Regression/Cases/test_c5_soi_trong_goi.py`: dựng một
...
**3.1.** Giải nén `ktc-quan-tri.skill`, đặt 10 tệp không có bản sao vào **`20-Chuan-Chung/`** — đúng vai trò
...
GĐ2  bổ sung 3 tệp, đóng gói lại  ──►  C5 xanh THẬT · C4 còn cảnh báo là bình thường
        │
GĐ3  đưa 10 tệp ra 20-Chuan-Chung ──►  C11 xanh
```
...
- **Không sửa nội dung nghiệp vụ của 3 tệp dùng chung.** Đợt này chỉ đồng bộ bản đã có ở `20-Chuan-Chung`
...
· `29-Cong-Cu/kiem_tra_he_thong.py` C4/C5 · `29-Cong-Cu/dong_goi_skill.py`
...
3. `references/01-Nguyen-Tac-Chung.md` trong gói là **bản cũ** của `20-Chuan-Chung/00-Nguyen-Tac-Chung.md`,
