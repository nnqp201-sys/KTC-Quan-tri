...
`python 29-Cong-Cu/kiem_tra_he_thong.py` báo **12 lỗi C5** (5 tệp dùng chung lệch bản gốc trong gói) + cảnh báo
C11 (1 tệp mồ côi). Sửa bằng cách nào: đóng gói lại từ đầu, hay có sẵn bản đã đúng?

## Phát hiện

Trên đĩa đã có 4 gói `.skill` **mới hơn** gói mà `29-Cong-Cu/kiem_tra_he_thong.py` đang trỏ tới, build cùng một
...
3. **5 tệp dùng chung trong gói khớp byte-for-byte với `20-Chuan-Chung/`** — đây chính là lỗi C5, nay đã hết.
...
| Bước | Kết quả |
|---|---|
| Trỏ `29-Cong-Cu/kiem_tra_he_thong.py` (dict `HE`) | 4 hệ → gói mới ở bảng trên; `ktc-quan-tri` giữ nguyên (đã đồng bộ) |
| Cập nhật `90-Nhat-Ky-Van-Hanh/MEMORY-INDEX.md` | Bảng "Phiên bản hiện hành" ghi đúng gói mới; sửa mâu thuẫn nội bộ "10 phép kiểm" → khớp "11 phép kiểm" |
...
| Xác nhận lại | `python 29-Cong-Cu/kiem_tra_he_thong.py --chi-tiet` → 0 lỗi; bộ thử ngược `test_kiem_tra_he_thong.py` chạy mã 0 |
...
Không đụng tới `92-Kinh-Nghiem/05-Known-Issues/Pending.md` — các việc mở ở đó (KI-001, 002, 003, 006, 007,
