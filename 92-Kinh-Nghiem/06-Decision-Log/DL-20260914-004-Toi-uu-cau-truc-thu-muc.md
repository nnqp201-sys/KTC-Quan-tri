...
|---|---|---|
| Dữ liệu đầu vào | `23-KTC-Ke-Hoach/Nhap_Ke_Hoach/` + `25-KTC-Bao-Cao/Nhap_Bao_Cao/` | `10-Dau-Vao/` |
| Sản phẩm xuất ra | `30-Ket-Qua/` + `25-KTC-Bao-Cao/Xuat_Bao_Cao/` + `23-KTC-Ke-Hoach/Xuat_Ke_Hoach/` | `30-Ket-Qua/YYYY-MM-DD/` |
| Bộ nhớ của hệ Báo cáo | `25-KTC-Bao-Cao/memory/` + `25-KTC-Bao-Cao/references/Memory/` | `references/Memory/` |

Ba chỗ "thứ hai" đều ra đời vì tiện lúc đó, rồi ở lại. Mỗi chỗ thừa là một chỗ để quên.

## Sáu việc đã làm

1. **Đầu ra về `30-Ket-Qua/`.** `Xuat_Bao_Cao/` và `Xuat_Ke_Hoach/` bỏ. Đây vốn là mâu thuẫn nội bộ:
   `20-Chuan-Chung/00-Nguyen-Tac-Chung.md` và `CLAUDE.md` **đã** quy định `30-Ket-Qua/YYYY-MM-DD/` là nơi
   duy nhất, nhưng `23-KTC-Ke-Hoach/SKILL.md` lại chỉ sang `Xuat_Ke_Hoach/`. Nay thống nhất theo bản gốc.
2. **Đầu vào về `10-Dau-Vao/`.** 40 tệp của `Nhap_Bao_Cao` đã chuyển, tên chuẩn hóa theo nội dung
   (`BC-KQ-thang-8-2026.xlsx`, `KH-thang-9-2026.xlsx`), thư mục mang **mã đơn vị chuẩn**. Bảng ánh xạ
   từng tệp: `30-Ket-Qua/2026-09-14/Anh-xa-chuyen-Nhap_Bao_Cao.md`.
3. **Hai lớp bộ nhớ của KTC-Bao-Cao gộp một.** `RUN-RECORD-SCHEMA.md` → `references/Memory/`;
   `KTC-RIS-Memory-Architecture.docx` → `91-Tai-Lieu-Thiet-Ke/` vì đó là tài liệu kiến trúc, không phải
...
5. **`29-Cong-Cu/narrative.json` ra khỏi thư mục mã nguồn** → `29-Cong-Cu/_trung_gian/`. Dữ liệu trung gian nằm lẫn
   với mã nguồn là chỗ dễ commit nhầm và khó biết xóa được hay không.
6. **Bỏ đường dẫn tuyệt đối trong `29-Cong-Cu/`.** `build_xl.py` và `trich_tuong_thuat.py` nay suy ra gốc dự án
...
| Công cụ | Kết quả |
|---|---|
| `29-Cong-Cu/build_xl.py` | Kế hoạch quý III → tháng 8: 41 nhiệm vụ, tháng 9: 26. Dựng được 2 tệp `.xlsx` |
| `29-Cong-Cu/trich_tuong_thuat.py` | **13 đơn vị**, 198 ý kết quả, 130 ý kế hoạch — nay hiện theo **mã chuẩn** (`P-QLKH`, `P-TCCB`…) thay vì tên thư mục tự do |
...
`30-Ket-Qua/2026-09-14/Danh-muc-can-don-toan-he.md`.
