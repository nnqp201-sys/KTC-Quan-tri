...
| Vai trò | Nguồn |
|---|---|
| **Đầu vào** | `KTC-Bao-Cao/Nhap_Bao_Cao/input-BC-Thang/` — 13 đơn vị · 13 `.docx` (Phụ lục IIa) + 27 `.xlsx` (IIb/Ib) |
...
```bash
python tools/trich_tuong_thuat.py && python tools/build_bc2.py && python tools/build_xl.py
python tools/chay_thu_ky.py --ghi
```
...
Sửa: `tools/chuan_bao_cao.py` **rút danh mục thẳng từ tệp mẫu**, không gõ tay — mẫu đổi thì code đổi theo.
