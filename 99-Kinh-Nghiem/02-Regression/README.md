...
```bash
python tools/kiem_tra_he_thong.py            # gọn
python tools/kiem_tra_he_thong.py --chi-tiet # xem cả cảnh báo
```
...
| **C5** 5 tệp dùng chung | bản sao phải khớp bản gốc `01-Chuan-Chung/` | lệch ~1 tháng ở hệ soạn thảo |
...
```bash
python 99-Kinh-Nghiem/02-Regression/Cases/test_kiem_tra_he_thong.py
```
...
| Vai trò | Tệp |
|---|---|
| Đầu vào | `KTC-Bao-Cao/Nhap_Bao_Cao/input-BC-Thang/` — 13 đơn vị, 13 `.docx` + 27 `.xlsx` |
...
1. Chạy pipeline trên bộ đầu vào tháng 8: `python tools/trich_tuong_thuat.py` → `build_bc2.py` → `build_xl.py`.
...
```
1. python tools/kiem_tra_he_thong.py                              → phải 0 lỗi
2. python 99-Kinh-Nghiem/02-Regression/Cases/test_*.py            → phải mã 0
3. Tầng 2: chạy pipeline tháng 8, ghi một dòng vào bảng chỉ số
4. Đóng gói lại .skill (tools/dong_goi_skill.py — GỘP, không ghi đè)
...
- **Chưa có bảng chỉ số theo thời gian** cho KTC-Quan-tri như 897 đã có. Cần dựng
  `99-Kinh-Nghiem/02-Regression/BANG-CHI-SO.md` và ghi dòng đầu tiên từ lần chạy 14/9/2026.
- **Chưa tự động hóa Tầng 2** — hiện chạy tay ba script rồi đối chiếu mắt. Nên gộp thành một
  `tools/chay_thu_ky.py` xuất thẳng bảng chỉ số.
