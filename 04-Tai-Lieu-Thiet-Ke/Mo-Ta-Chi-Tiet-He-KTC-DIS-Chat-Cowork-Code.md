...
- **Cơ chế kỹ thuật đã tích hợp:**
  - *v2.15* — đọc thuộc tính định dạng thật (`tools/_read_input_properties.py`, python-docx: `section.*_margin`, `run.font.size`/`bold`) thay vì chỉ đọc `paragraph.text` → khắc phục việc mọi lỗi định lượng (lề sai, cỡ chữ sai) từng bị bỏ sót.
...
- **Cấu trúc thư mục:** `00-README.md`, `SKILL.md`, `ktc-ra-soat-897-v2.17.skill` · `assets/` · `references/` (Checklist 01–08, Skill-Library, Prompt-Library) · `tools/` (`build_ktc897_report.py` v3.0, `_read_input_properties.py`) · `12-Output/` (nguồn tự học) · `13-Regression-Test/` · `09-Quan-Tri-He/10-Bo-Nho-He/`.
...
**Nguồn sự thật duy nhất (`SOURCE-OF-TRUTH.md`):** `core/` ở cấp repo là nguồn nghiệp vụ duy nhất; `plugin/core/` là **snapshot sinh ra** bởi `tools/build_universal.py`; `dist/` là artifact sinh ra, không sửa tay; v2.x/v3.0 chỉ để so sánh/rollback, không bị build Universal ghi đè.
