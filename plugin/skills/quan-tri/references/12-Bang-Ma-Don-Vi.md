...
1. **`Phong-TCCB-CTHHSV` sai chính tả** — thừa một chữ `H`. Đúng phải là `CTHSSV` (Công tác học sinh,
   sinh viên). Thư mục này nằm trong `KTC-Ke-Hoach/Nhap_Ke_Hoach/input-KH_Nam/` và `input-KH_Quy/`.
   Đổi tên thư mục sẽ kéo theo phải sửa `KTC-Ke-Hoach/SKILL.md` và đóng gói lại `.skill` — xếp vào
...
Ghi chú `04-Tai-Lieu-Thiet-Ke/GHI-CHU-CAU-NOI-3-HE.md` (19/8/2026) từng kết luận tên đơn vị "khớp hoàn
